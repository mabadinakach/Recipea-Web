...
document.getElementById("search-button").addEventListener("click", function() {
    ...
    sessionStorage.setItem('ingredients', ingredientsArrayFilter);
    for(var i = 0; i<ingredientsArrayFilter.length; i++) {
        if (i>0) {
            ingredientsArrayFilter[i] = "+" + ingredientsArrayFilter[i]
        }
    }
    sessionStorage.setItem('ingredients', ingredientsArrayFilter);
});
...
document.getElementById("searchFilterButton").addEventListener("click", function() {
  ...
  let filteredIngredientsList = ingredientsList.filter(word => word.toLowerCase().includes(search.toLowerCase()))
  var div = document.getElementById('ingredients-items');
  while(div.firstChild){
    div.removeChild(div.firstChild);
  }
